...
export const VendorCard = ({
  ...
  const image_url = `/images/placeholders/vendors/img_${Math.ceil(
    Math.random() * 10
  )}.svg`;
  ...
  return (
    ...
    >
      ...
      <CardBody p="0rem">
        ...
        >
          <Image
            // src={vendor_details?.image_url}
            src={
              vendor_details?.images?.find((el: any) => el.is_featured == '1')
                ?.image ||
              image_url ||
        ...
        <Flex justify={'space-between'} p="1rem">
          <Stack>
            <Heading
              ...
              lineHeight={'2.2rem'}
            >
              {vendor_details?.name || 'Walmart' || 'Unknown Product'}
            </Heading>
            <Text
              color={'#1C1C1C'}
              fontWeight={'600'}
            ...
            >
              ...
              }).format(vendor_details?.price || 0)}
            </Text>
            <Flex align={'center'} gap={'1rem'}>
              <StarRatings
                rating={
                  (vendor_details?.rating /
                    vendor_details?.best_possible_rating) *
                    5 || 3.5
                }
              />
              <Text color={'#FF9017'}>{vendor_details?.rating || 7}</Text>
            </Flex>
          </Stack>
